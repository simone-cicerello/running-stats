...
public class StravaService {
    ...
    public AthleteModel getAthlete() {
        try {
            ...
            HttpRequest request = HttpRequest.newBuilder()
                    .uri(new URI(url))
                    .header("Authorization", "Bearer " + sTokenService.getStravaToken(false))
                    .GET()
                    .build();
            var athleteResponseWrap = HttpClient.newHttpClient().send(request, HttpResponse.BodyHandlers.ofString());
    ...
    public List<SummaryActivityModel> getAthleteActivities(Date before, Date after, Integer page, Integer perPage) {
        try {
            var url = stravaConfig.getStravaBaseUrl() + "/" + stravaConfig.getStravaAthleteActivitiesUrl();
            log.info("STRAVA - calling {}", url);
            ...
            var athleteActivitiesResponseWrap = HttpClient.newHttpClient().send(request, HttpResponse.BodyHandlers.ofString());

            return new ObjectMapper().readValue(athleteActivitiesResponseWrap.body(), new TypeReference<List<SummaryActivityModel>>() {});
        } catch (URISyntaxException | IOException | InterruptedException e) {
            log.error(e.getMessage());
            throw new ServiceException(e.getMessage(),
                    e.getCause(),
